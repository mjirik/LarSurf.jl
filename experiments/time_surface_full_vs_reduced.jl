...
larmodel3 = lario3d.get_surface_grid_per_block_Vreduced_FVreduced_old(segmentation, [2,2,2])
...
function run_measurement(segmentation_size_factor, b3_size; skip_full=false)
    ...
    save_data("step Vr and FVr", tmd, segmentation, b3_size)
    tmd = @timed(lario3d.grid_Fchar_to_Vreduced_FVreduced_old(Fchar, data_size))
    save_data("step Vr and FVr old", tmd, segmentation, b3_size)
    tmd = @timed(lario3d.grid_Fchar_to_V_FVreduced(Fchar, data_size))
    save_data("step V and FVr", tmd, segmentation, b3_size)
    if ~skip_full
    ...
    tmd = @timed(lario3d.get_surface_grid_per_block_Vreduced_FVreduced_old(segmentation, b3_size))
    save_data("seg Vr and FVr old", tmd, segmentation, b3_size)
